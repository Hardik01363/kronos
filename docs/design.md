...
4. Priority: This value will not be given by User and its initial default value will be set to 0. This  value will be used and dynamically changed by the MLFQ algorithm.
5. enum State: The state enum will have 3 possible states to be in (ready, running, completed). We have no need for a blocked state as we are not considering I/O in our metrics calculations.
